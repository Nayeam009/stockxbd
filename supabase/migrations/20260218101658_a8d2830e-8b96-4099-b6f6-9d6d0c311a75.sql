...
-- Add stock availability pre-check before inventory update

CREATE OR REPLACE FUNCTION public.complete_pos_sale(p_transaction_number text, p_customer_id uuid DEFAULT NULL::uuid, p_subtotal numeric DEFAULT 0, p_discount numeric DEFAULT 0, p_total numeric DEFAULT 0, p_payment_method payment_method DEFAULT 'cash'::payment_method, p_payment_status text DEFAULT 'paid'::text, p_notes text DEFAULT NULL::text, p_is_online_order boolean DEFAULT false, p_community_order_id uuid DEFAULT NULL::uuid, p_items jsonb DEFAULT '[]'::jsonb, p_return_items jsonb DEFAULT '[]'::jsonb, p_remaining_due numeric DEFAULT 0)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
