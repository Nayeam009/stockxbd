...
export const MobileBottomNav = ({ activeModule, setActiveModule, userRole }: MobileBottomNavProps) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-card/95 backdrop-blur-xl border-t border-border/50 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
      {/* Swipe indicator */}
      <div className="flex items-center justify-between px-4 py-1.5 border-b border-border/30 bg-muted/30">
        <button 
          onClick={handlePrevModule}
          className="p-2 -m-1 rounded-full hover:bg-muted active:scale-95 transition-all touch-target flex items-center justify-center"
          aria-label="Previous module"
        >
          <ChevronLeft className="h-5 w-5 text-muted-foreground" />
        </button>
        <div className="flex items-center gap-1.5" role="tablist" aria-label="Module navigation">
          {modules.slice(Math.max(0, currentIndex - 2), currentIndex + 3).map((_, idx) => {
            ...
            return (
              <div 
                key={actualIdx}
                role="tab"
                aria-selected={actualIdx === currentIndex}
                className={cn(
      ...
      <div className="flex items-center justify-around h-16 px-1">
        {filteredNavItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(
                "text-[10px] font-medium mt-1 truncate max-w-[60px]",
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          ...
          <SheetContent side="bottom" className="h-[70vh] rounded-t-3xl" style={{ paddingBottom: 'env(safe-area-inset-bottom, 16px)' }}>
