...
import { Users, Banknote, TrendingUp, Truck, Receipt, Wallet, ClipboardList, ChefHat, Wrench, RefreshCw, Tag, BarChart3, Settings, Package } from "lucide-react";
...
export const DashboardOverview = ({
  ...
  return <div className="space-y-4 sm:space-y-6 lg:space-y-8 px-1 sm:px-2 md:px-0">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 lg:gap-6">
        ...
        const iconGradients = ['from-primary to-primary-light', 'from-secondary to-secondary-light', 'from-accent to-accent-light', 'from-success to-accent'];
        return <Card key={index} className="group relative overflow-hidden border border-border/40 shadow-md sm:shadow-lg hover:shadow-xl transition-all duration-500 bg-card hover:-translate-y-1 sm:hover:-translate-y-2">
              ...
              <CardContent className="p-3 sm:p-4 pt-0">
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                    <Badge className={`${stat.changeType === 'positive' ? 'bg-success/15 text-success border-success/30' : 'bg-destructive/15 text-destructive border-destructive/30'} font-semibold text-[10px] sm:text-xs border px-1 sm:px-1.5`}>
      ...
      {(userRole === 'owner' || userRole === 'manager') && drivers.length > 0 && <Card className="border border-border/40 shadow-md sm:shadow-lg bg-card overflow-hidden">
          ...
          <CardContent className="p-3 sm:p-4 lg:pt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {drivers.map(driver => <div key={driver.id} className="p-3 sm:p-4 bg-muted/30 rounded-lg sm:rounded-xl border border-border/40 hover:border-primary/40 hover:shadow-md transition-all duration-300 group">
                  <div className="flex items-center justify-between mb-2 sm:mb-3 gap-2">
                    ...
                    <Badge className={`text-[10px] sm:text-xs font-semibold border flex-shrink-0 ${driver.status === 'active' ? 'bg-success/15 text-success border-success/30' : driver.status === 'break' ? 'bg-warning/15 text-warning border-warning/30' : 'bg-muted text-muted-foreground border-border'}`}>
                      {driver.status}
                    </Badge>
                  </div>
      ...
      <Card className="border border-border/40 shadow-md sm:shadow-lg bg-card overflow-hidden">
        ...
        <CardContent className="p-3 sm:p-4 lg:pt-6">
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-2 sm:gap-3 lg:gap-4">
            ...
            return <button key={index} onClick={() => handleQuickAction(action.module)} className="group flex flex-col items-center justify-center p-2.5 sm:p-4 lg:p-5 rounded-lg sm:rounded-xl border border-border/40 bg-muted/20 hover:bg-primary/10 hover:border-primary/40 hover:shadow-lg transition-all duration-300 min-h-[70px] sm:min-h-[90px] lg:min-h-[100px]">
